...
class DatasetSpec:
    ...
    # Licensed/optional source (WRDS): not ingesting it is not a validation failure.
    optional: bool = False
...
_SPECS = [
    ...
    DatasetSpec(
        ...
        description="World Bank Pink Sheet monthly commodity prices (monthly averages, nominal)",
        columns={
            "series": "string",
            "unit": "string",
            "date": DT,
    ...
    DatasetSpec(
        ...
        description="Monthly total returns for the 18 asset classes: ETF spliced with public proxies",
        columns={
            "asset_id": "string",
            "date": DT,
            "ret": "float64",
    ...
    DatasetSpec(
        name="market/asset_history_links",
        description="Coverage and tracking vs. the ETF for every link in each asset's history chain",
        columns={
            "asset_id": "string",
            "priority": "int64",
            "source": "string",
            ...
            "last_date": DT,
            "months_used": "int64",
            "overlap_months": "int64",
            "corr": "float64",
            "tracking_error": "float64",
            "mean_diff": "float64",
        },
        keys=("asset_id", "priority"),
        date_col=None,
        entity_col="asset_id",
    ),
    # ---- licensed WRDS data: stays in git-ignored data/, never committed or published
    DatasetSpec(
        name="wrds/crsp_treasury_indexes",
        description="CRSP monthly fixed-term Treasury and T-bill index returns (licensed, WRDS)",
        columns={"series": "string", "date": DT, "ret": "float64", "available_from": DT},
        keys=("series", "date"),
        entity_col="series",
        optional=True,
    ),
    DatasetSpec(
        name="wrds/crsp_stock_monthly",
        description="CRSP monthly total returns, prices and shares for selected securities (licensed)",
        columns={
            "permno": "int64",
            "ticker": "string",
            "date": DT,
    ...
    DatasetSpec(
        name="wrds/crsp_fund_monthly",
        description="CRSP mutual fund monthly total returns and net assets (licensed, WRDS)",
        columns={
            "ticker": "string",
            "crsp_fundno": "int64",
            "date": DT,
